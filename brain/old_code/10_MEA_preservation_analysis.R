...
domExp <- readRDS("manuscript/brain/manuscript70/results/wgcna/datExpr_MEA_CDOM.RDS")

domColor <-readRDS("manuscript/brain/manuscript70/results/kIN/MEA_CDOM_kIN_dataframe_.RDS")
domColor <- domColor %>% dplyr::select(ensgene, module) %>% dplyr::rename(.,geneID = ensgene)%>% dplyr::rename(.,Module = module)
  
subExp <- readRDS("manuscript/brain/manuscript70/results/wgcna/datExpr_MEA_CSUB.RDS")

subColor <-readRDS("manuscript/brain/manuscript70/results/kIN/MEA_CSUB_kIN_dataframe_.RDS")
...
# library(flashClust)
# #first doms
# dissTOM_dom = 1-TOMsimilarityFromExpr(domExp[,domColor$geneID], power = 6, TOMType = "unsigned")
...
# Save the results
saveRDS(mp,"manuscript/brain/manuscript70/results/RDS/MEA_modulePreservation2.RDS")

# mp <- readRDS("manuscript/brain/manuscript70/results/RDS/MEA_modulePreservation.RDS") dom and sub colors
mp2 <-readRDS("manuscript/brain/manuscript70/results/RDS/MEA_modulePreservation2.RDS") #just dom colors
...
# Start the plot
sizeGrWindow(10, 5)
# png("manuscript/brain/manuscript70/results/results_figures/MEA_DOMonly-modulePreservation-Zsummary-medianRank.png", wi=10, h=5,units = 'in', res = 300)
par(mfrow = c(1,2))
...
# sizeGrWindow(10, 9);
png("manuscript/brain/manuscript70/results/results_figures/MEA_DOMonly-PreservationZStatistics.png", wi=10, h=9,units = 'in', res = 300)
par(mfrow = c(4,4))
par(mar = c(3,3,2,1))
par(mgp = c(1.6, 0.4, 0));
...
hub <- read_csv("manuscript/brain/manuscript70/results/wgcna/wgcna_table/MEA_CDOM_hubgene_list.csv")
head(hub)

hub$moduleName

#blue
b_hub <- hub %>%  filter(moduleName == "magenta") %>% arrange(symbol) %>% unique(.)
b <- b_hub$symbol
...
hub <- readRDS("manuscript/brain/manuscript70/results/wgcna/MEA_CSUB_WGCNA_MM_GS_all.RDS")
head(hub)
...
xx <- limma_list<- readRDS("manuscript/brain/manuscript70/results/RDS/limma_MEA_ControlSUB.RDS") %>% 
  map(~distinct(.)) %>% 
  # map(~filter(.,abs(logFC) >= .2)) %>%
  # map(~filter(.,P.Value <0.05)) %>% 
  map(~ left_join(., grcm38 %>% dplyr::select(symbol))) %>% 
