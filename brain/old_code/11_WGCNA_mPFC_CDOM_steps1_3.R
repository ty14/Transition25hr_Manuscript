...
saveRDS(datExpr,"manuscript/brain/manuscript70/results/WGCNA/datExpr_mPFC_CDOM.RDS")
# ===========================================================================
...
WGCNA_get_net <- function(my_tissue = "mPFC70min",
  ...
  x <- readRDS("manuscript/brain/manuscript70/results/WGCNA/datExpr_mPFC_CDOM.RDS") 
  ...
  saveRDS(net, "manuscript/brain/manuscript70/results/WGCNA/net_mPFC_CDOM_Power5.RDS")
...
net <- readRDS(glue::glue("manuscript/brain/manuscript70/results/WGCNA/net_mPFC_CDOM_Power5.RDS"))
...
png(file = "manuscript/brain/manuscript70/results/results_figures/cluster_dendo_mPFC_CDOM_Power5.png",
    width=600, height=350)
plotDendroAndColors(net$dendrograms[[1]], mergedColors[net$blockGenes[[1]]],
                    "Module colors",
                    dendroLabels = FALSE, hang = 0.03,
                    addGuide = TRUE, guideHang = 0.05, 
                    main = "mPFC CDOM Power5")
...
#
#=====================================================================================
x <- coldata %>% filter(region != "AMY")

MEs <- read_csv("manuscript/brain/manuscript70/results/wgcna/MEA_CDOM_MEs.RDS")

ME_df <-MEs%>% data.frame() %>% 
  tibble::rownames_to_column(var = "SampleName") %>%
  pivot_longer(cols = 2:46, names_to = "Module") %>% 
  full_join(x)
...
x <- ME_df %>%  filter(Module == "steelblue")
x <- ME_df %>%  filter(Module == "sienna3")
x <-ME_df %>%  filter(Module == "violet")
...
x %>%
  ...
  labs(x = "Social status",
       y = "Module eigengene",
       title = "mPFC: Violet")+ theme_classic() +   theme(legend.position = "none")
...
datExpr <- readRDS("manuscript/brain/manuscript70/results/wgcna/datExpr_mPFC_CDOM.RDS") 
net <- readRDS("manuscript/brain/manuscript70/results/wgcna/net_mPFC_CDOM_Power5.RDS")
MEs <- readRDS("manuscript/brain/manuscript70/results/wgcna/MEA_mPFC_MEs.RDS")
...
saveRDS(lm_result_all,"manuscript/brain/manuscript70/results/RDS/mPFC_CDOM_lm_result_all.RDS")
