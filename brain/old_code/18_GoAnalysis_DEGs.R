...
y1a <- limma_list$controldom

y2a <- limma_list$controldes

y3a <- limma_list$domdes
...
y1b <- limma_list$controldom

y2b <- limma_list$controldes
 
y3b <- limma_list$domdes
...
gettop10GO(y1a, my_showCategory) %>% 
  mutate(comparison = "CDOM-DOM") -> top10go1

gettop10GO(y2a, my_showCategory ) %>% 
  mutate(comparison = "CDOM-DES") -> top10go2

gettop10GO(y3a, my_showCategory ) %>% 
  mutate(comparison = "DOM-DES") -> top10go3


rbind(top10go1,top10go2,top10go3) -> top10_GOterms

write.csv(top10_GOterms,"manuscript/brain/manuscript70/results/tables/topBP_GOterms_abovelogFC0.2_MeA_CDOM70min.csv", row.names = F)
...
rbind(top10go1,top10go2,top10go3) -> top10_GOterms
 ...
 write.csv(top10_GOterms,"manuscript/brain/manuscript70/results/tables/topBP_GOterms_abovelogFC0.2_mPFC_CDOM70min.csv", row.names = F)
...
dotplot(up, showCategory=5) + 
  ggtitle("Upregulated gene expression in Dominant Go Terms") +
