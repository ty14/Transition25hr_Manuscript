...
dlNorm <-as.data.frame(p_count)

dlNorm <- dlNorm  %>% dplyr::select(-B1.PFCB12.3.4.trim.sam.counts)
...
coldata <- coldata %>% filter(SampleName != 'B1.PFCB12.3.4.trim.sam.counts')
...
saveRDS(v.dl, "manuscript/brain/manuscript70/results/RDS/limma_vdl_mPFC_ControlDD.RDS")
...
saveRDS(q.dl,("manuscript/brain/manuscript70/results/RDS/limma_vdl_cutoff5_2000_tworand_mPFC_70_controlDD.RDS"))

q.dl <- readRDS("manuscript/brain/manuscript70/results/RDS/limma_vdl_cutoff5_2000_tworand_mPFC_70_controlDD.RDS")

#

png(filename ="manuscript/brain/manuscript70/results/results_figures/eFDR_hist_mPFC_R5000_ControlDD.png",
    width = 18, height = 17, units = "cm", res = 300)
hist(q.dl-p.dl.limma2, main = "CDOM mPFC")
invisible(dev.off())
...
topTable(tmp1, sort.by = "P", n = Inf) %>%
  ...
  left_join(grcm38) %>%
  filter(!is.na(symbol)) %>% 
  dplyr::select(symbol,logFC,P.Value,adj.P.Val) -> limma_list$controldom

topTable(tmp2, sort.by = "P", n = Inf) %>%
  rownames_to_column('ensgene') %>%
  left_join(grcm38) %>%
  filter(!is.na(symbol)) %>%
  dplyr::select(symbol,logFC,P.Value,adj.P.Val)  -> limma_list$controldes
...
saveRDS(limma_list,"manuscript/brain/manuscript70/results/RDS/limma_mPFC_ControlDD.RDS")
...
contrast.matrix <- makeContrasts(group.dlCSUB-group.dlSUB,
                                 group.dlCSUB-group.dlASC, 
                                 group.dlSUB-group.dlASC,
                                 levels=design.dl)
...
saveRDS(v.dl, "manuscript/brain/manuscript70/results/RDS/limma_vdl_mPFC_ControlSub.RDS")
...
saveRDS(q.dl,("manuscript/brain/manuscript70/results/RDS/limma_vdl_cutoff5_2000_tworand_mPFC_70_controlSub.RDS"))

q.dl <- readRDS("manuscript/brain/manuscript70/results/RDS/limma_vdl_cutoff5_2000_tworand_mPFC_70_controlSub.RDS")

#################

png(filename ="manuscript/brain/manuscript70/results/results_figures/eFDR_hist_mPFC_R5000_ControlSUB.png",
    width = 18, height = 17, units = "cm", res = 300)
hist(q.dl-p.dl.limma2, main = "CSUB mPFC")
invisible(dev.off())
...
topTable(tmp1, sort.by = "P", n = Inf) %>%
  ...
  left_join(grcm38) %>%
  filter(!is.na(symbol)) %>% 
  dplyr::select(symbol,logFC,P.Value,adj.P.Val) -> limma_list$controlsub

topTable(tmp2, sort.by = "P", n = Inf) %>%
  rownames_to_column('ensgene') %>%
  left_join(grcm38) %>%
  filter(!is.na(symbol)) %>%
  dplyr::select(symbol,logFC,P.Value,adj.P.Val)  -> limma_list$controlasc
...
topTable(tmp3, sort.by = "P", n = Inf) %>%
  ...
  left_join(grcm38) %>%
  filter(!is.na(symbol)) %>%
  dplyr::select(symbol,logFC,P.Value,adj.P.Val) -> limma_list$subasc

saveRDS(limma_list,"manuscript/brain/manuscript70/results/RDS/limma_mPFC_ControlSUB.RDS")
