...
# getting trait data
colnames(coldata)
amy <- coldata[c(3,5,6,13:16,16, 18:20,22)]
...
plotDendroAndColors(sampleTree2, traitColors,
                    ...
                    main = "mPFC dendrogram and trait heatmap(all samples)")

Samples = rownames(datExpr)
collectGarbage()

saveRDS(datExpr,"manuscript/brain/manuscript70/results/WGCNA/datExpr_mPFC_CSUB.RDS")
...
WGCNA_get_net <- function(my_tissue = "mPFC70min",
  ...
  x <- readRDS("manuscript/brain/manuscript70/results/WGCNA/datExpr_mPFC_CSUB.RDS") 
  ...
  saveRDS(net, "manuscript/brain/manuscript70/results/WGCNA/net_mPFC_CSUB_Power6.RDS")
...
net <- readRDS(glue::glue("manuscript/brain/manuscript70/results/WGCNA/net_mPFC_CSUB_Power6.RDS"))
...
png(file = "manuscript/brain/manuscript70/results/results_figures/cluster_dendo_mPFC_CSUB_Power6.png",
    width=600, height=350)
plotDendroAndColors(net$dendrograms[[1]], mergedColors[net$blockGenes[[1]]],
                    "Module colors",
                    dendroLabels = FALSE, hang = 0.03,
                    addGuide = TRUE, guideHang = 0.05, 
                    main = "mPFC CSUB Power6")
...
plotDendroAndColors(sampleTree2, traitColors,
                    groupLabels = names(amy1),
                    main = "mPFC dendrogram and trait heatmap SUB:ASC")
...
saveRDS(MEs,"manuscript/brain/manuscript70/results/wgcna/mPFC_CSUB_MEs.RDS")
...
png(file = "manuscript/brain/manuscript70/results/results_figures/module_trait_mPFC_CSUB.png",
    width=1200, height=1400, res = 130)
...
labeledHeatmap(Matrix = moduleTraitCor,
               ...
               cex.text = 0.5,
               zlim = c(-1,1),
               main = "CSUB mPFC 70min: Module-trait relationships")
#
#
dev.off()
...
ME_df <-MEs%>% data.frame() %>% 
  ...
  pivot_longer(cols = 2:42, names_to = "Module") %>% 
  full_join(coldata)
...
x <- ME_df %>%  filter(Module == "darkgreen")
...
ggsave(filename = "manuscript/brain/manuscript70/results/results_figures/mPFC_eigengene_CORT_CSUB.png",
       p1,
...
x %>%
  ...
  labs(x = "Social status",
       y = "Module eigengene",
       title = "mPFC: Darkgreen")+ theme_classic() + theme(legend.position = "none")

p2

ggsave(filename = "manuscript/brain/manuscript70/results/results_figures/mPFC_CSUB_eigengene_boxplot.png",
       p2,
...
ggsave(filename = "manuscript/brain/results/results_figures/MEA_eigengene_AggRec_CSUB.png",
 p3,
 height = 10, width = 10, dpi = 150)
...
saveRDS(wgcna_all, "manuscript/brain/manuscript70/results/wgcna/mPFC_CSUB_WGCNA_MM_GS_all.RDS")
...
module = "lightgreen"
module = "darkgreen"
module_list = c("black", "magenta", "pink", "blue", "sienna3", "skyblue", "cyan", "steelblue", 'lightgreen', "darkgreen")
...
for (module in module_list){
  ...
  hub_genes %>% 
    ...
    dplyr::rename(moduleMembership = glue::glue("MM{module}")) -> hub_genes
...
write.csv(hubgenes_df, "manuscript/brain/manuscript70/results/wgcna/wgcna_table/mPFC_CSUB_hubgene_list.csv", row.names = F)
...
saveRDS(kIN_df,"manuscript/brain/manuscript70/results/kIN/mPFC_CSUB_kIN_dataframe_.RDS")
...
png(glue("manuscript/brain/manuscript70/results/results_figures/MEA_CSUB_TOMplotsignedhybird_9.png"),   width = 12, height = 12, units = "cm", res = 600)
...
for(x in 1:length(MEs0)){
  ...
  rbind(summary(lm(df[,9] ~ df[,1]))$coefficients[2,],
    ...
    as.data.frame() %>%
    cbind(key = c("CSUB-SUB","CSUB_ASC","SUB-ASC")) %>%
    mutate(module = md) -> lm_result_list[[x]]
}
...
saveRDS(lm_result_all,"manuscript/brain/manuscript70/results/RDS/mPFC_CSUB_lm_result_all.RDS")

df <- readRDS("manuscript/brain/manuscript70/results/RDS/mPFC_CSUB_lm_result_all.RDS")
...
key_level <- c("CSUB-SUB","CSUB-ASC","SUB-ASC")
...
png(filename = "manuscript/brain/manuscript70/results/results_figures/traitmodule_MEA_SUBgroups.png",
...
ggsave(filename = "manuscript/brain/manuscript70/results/results_figures/modulesize_MEA_CSUB.png",temp_p,
...
write.csv(wgcna_all_gos, 
          "manuscript/brain/manuscript70/results/wgcna/wgcna_table/CSUB_mPFC_wgcna_all_gos_catogeryBP.csv",
          row.names = F)

saveRDS(wgcna_all_gos,"manuscript/brain/manuscript/results/wgcna/CSUB_mPFC_wgcna_all_gos.RDS")
