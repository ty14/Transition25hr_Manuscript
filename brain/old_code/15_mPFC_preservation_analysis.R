...
domExp <- readRDS("manuscript/brain/manuscript70/results/wgcna/datExpr_mPFC_CDOM.RDS")

domColor <-readRDS("manuscript/brain/manuscript70/results/kIN/mPFC_CDOM_kIN_dataframe_.RDS")
domColor <- domColor %>% dplyr::select(ensgene, module) %>% dplyr::rename(.,geneID = ensgene)%>% dplyr::rename(.,Module = module)
  
subExp <- readRDS("manuscript/brain/manuscript70/results/wgcna/datExpr_mPFC_CSUB.RDS")

subColor <-readRDS("manuscript/brain/manuscript70/results/kIN/mPFC_CSUB_kIN_dataframe_.RDS")
...
# Save the results
saveRDS(mp,"manuscript/brain/manuscript70/results/RDS/mPFC_modulePreservation.RDS")

mp <- readRDS("manuscript/brain/manuscript70/results/RDS/mPFC_modulePreservation.RDS") #dom and sub colors
...
# sizeGrWindow(10, 9);
png("manuscript/brain/manuscript70/results/results_figures/mPFC_DOMonly-PreservationZStatistics.png", wi=10, h=9,units = 'in', res = 300)
par(mfrow = c(4,4))
par(mar = c(3,3,2,1))
par(mgp = c(1.6, 0.4, 0));
