...
p70_down <- p %>% filter(symbol %in% c("Kcns3", "Crybg2", "Stx19", "Cmah", "Cndp1", "Dhx16", "Zfp280d", 'Chka', 'Kmt2a', 'Sfmbt1', 'Plekha6'))
...
p1 <- ggplot(p70_up, aes(group,value, color = group, fill = group))+
  ...
  scale_color_manual(values = viridis::viridis(4))+
  scale_fill_manual(values = viridis::viridis(4))+
  facet_wrap(~symbol, scales = 'free', ncol =11)+
  scale_y_continuous(expand = c(0, 1))+
  ylab("Normalized Expression") +
  xlab("")+
  theme_bw()+
...
ggsave("manuscript/brain/imgs/TRN70_UPreg_genes_boxplots.png",p1, width =16 , height = 2.5, dpi = 300)
...
ggsave("manuscript/brain/imgs/TRN70_DOWNreg_genes_boxplots.png", width =22 , height = 2.5, dpi = 300)
