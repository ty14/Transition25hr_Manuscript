...
pfc_data<- coldata %>% filter(region != "AMY") %>% filter(Prerank !=3) %>% 
  filter(groupEX != "control") %>% filter(Postrank != 3) %>% filter(condition1 != "SUB")
...
pfc_data1$condition1 %>%
  factor(.,levels = c("DES", "DOM","ASC")) -> group.dl
...
select <- order(rv, decreasing = TRUE)[1:300] #top 250 varying genes
...
ggsave("manuscript/brain/imgs/pca_mPFC_70mn_man_opt2A.png", p1, width = 6, height = 5)
...
# Second mpfc data data 
pfc_data<- coldata %>% filter(region != "AMY") %>% filter(Prerank !=3) %>% 
  filter(groupEX != "control") %>% filter(Postrank != 3) %>% filter(condition1 != "DOM")
...
##making it a factor
pfc_data1$condition1 %>%
  factor(.,levels = c("DES", "SUB","ASC")) -> group.dl
...
#get expression levels
rv <- rowVars((v$E)) #measure of variation for each gene
select <- order(rv, decreasing = TRUE)[1:450] #top 250 varying genes
pca1 <- prcomp(t((v$E)[select, ])) #do pca
...
p1=ggplot(d1, aes(PC1, PC2, color = condition1, shape = condition1))+
  geom_point(size = 5)+
  scale_shape_manual(values=c(16,17,3,3,7,8,1))+
  labs(x = xlab, y = ylab, color = "condition", shape = "condition")+
  scale_color_manual(values = viridis::viridis(6))+
  theme_dose()+ theme(text = element_text(size = 20))+
  ggtitle('B. 70 min PCA')

p1

ggsave("manuscript/brain/imgs/pca_mPFC_70mn_man_opt2B.png", p1, width = 6, height = 5)
...
#getting condition1
table(coldata$condition, coldata$Postrank)
#fixing sub mouse with no condition
...
table(coldata$condition1)
# Second mpfc data data 
pfc_data<- coldata %>% filter(region != "A") %>% filter(condition1 != 'SUB')

#b1.2.1 outlier DES
pfc_data1<- pfc_data[c(1:13,15:18),]

row.names <- pfc_data1$SampleNames

pfc_data1 <- pfc_data1%>% dplyr::select(-SampleNames)
...
# Bring in count data for mpfc
p_countdata <- read_csv("brain/PFC_25hcounts.csv")
p_countdata$X -> nrows
p_count <- p_countdata[,-1]
row.names(p_count) <- nrows
p_count <-as.data.frame(p_count)
...
#get expression levels
rv <- rowVars((v$E)) #measure of variation for each gene
select <- order(rv, decreasing = TRUE)[1:200] #top 250 varying genes
pca1 <- prcomp(t((v$E)[select, ])) #do pca
...
ggsave("manuscript/brain/imgs/pca_mPFC_70mn_man_opt2C.png", p1, width = 6, height = 5)
...
table(coldata$condition1)
# Second mpfc data data 
pfc_data<- coldata %>% filter(region != "A") %>% filter(condition1 != 'DOM')

#b1.2.1 outlier DES
pfc_data1<- pfc_data[c(1:15,17,18),]
...
#get expression levels
rv <- rowVars((v$E)) #measure of variation for each gene
select <- order(rv, decreasing = TRUE)[1:100] #top 250 varying genes
pca1 <- prcomp(t((v$E)[select, ])) #do pca
...
ggsave("manuscript/brain/imgs/pca_mPFC_70mn_man_opt2D.png", p1, width = 6, height = 5)
