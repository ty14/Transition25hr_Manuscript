...
vp_ad <- ggplot(data = dc, 
  ...
  geom_text_repel(data = dcx, aes(x = logFC, y = -log10(P.Value),label = symbol), color = "black", vjust = 1.5,  hjust = -.3)+
  geom_text_repel(data = dcxx, aes(x = logFC, y = -log10(P.Value),label = symbol), color = "black", hjust = 0.0,vjust = -.6)+
...
dcx <- dc %>% filter(.,logFC >= 1.8) %>% filter(P.Value < 0.05)
...
vp_dd <- ggplot(data = dc, 
  ...
  geom_vline(xintercept=c(-1.5,1.5),lty=4,col="black",lwd=0.8) +
  geom_hline(yintercept = 1.301,lty=4,col="black",lwd=0.8) +
  geom_text_repel(data = dcx, aes(x = logFC, y = -log10(P.Value),label = symbol), color = "black", vjust = 0.8,  hjust = -0.9)+
...
vp_as <- ggplot(data = dc, 
  ...
  geom_text_repel(data = dcxx, aes(x = logFC, y = -log10(P.Value),label = symbol), color = "black", hjust = -0.65, vjust = -0.65)+
  labs(x="log2 Fold Change",
       y=bquote(~-Log[10]~italic(eFDR)))  +
  theme_bw() +
  annotate(geom="text", x=2.15, y=.5, label=paste0(" ", "\n", "ASC vs. SUB"),
...
dcx <- dc %>% filter(.,logFC >= 2.1) %>% filter(P.Value < 0.05)
...
vp_ds <-ggplot(data = dc, 
  ...
  geom_vline(xintercept=c(-1.5,1.5),lty=4,col="black",lwd=0.8) +
  geom_hline(yintercept = 1.301,lty=4,col="black",lwd=0.8) +
  geom_text_repel(data = dcx, aes(x = logFC, y = -log10(P.Value),label = symbol), color = "black", vjust = 0.75,  hjust = -0.75)+
  geom_text_repel(data = dcxx, aes(x = logFC, y = -log10(P.Value),label = symbol), color = "black", hjust = -0.4, vjust = -0.5)+
  labs(x="log2 Fold Change",
       y=bquote(~-Log[10]~italic(eFDR)))  +
  theme_bw() +
  annotate(geom="text", x=2.15, y=.5, label=paste0(" ", "\n", "DES vs. SUB"),
