...
df %>%
  ...
  .$symbol -> for_label
for_label <- c( "Mapkapk2", "Map2k7", "Nfkb1", "Adam17", "Lif", "Foxo3")
...
df %>%
 ...
 df %>% filter(Sig != "N.S.")
for_label <- for_label <- c( "Foxo3", "Trim12c", "Il18r1",  "Adam17",  "Lif", "Nfkb1",   'Prlr', "Kpna4", "Rapgef1") 
...
df %>% filter(Sig != "N.S.")
for_label<-  c( "Mapkapk2", "Il5ra", "Vrk3","Nfkb1",'Pomc','Txlna', "Isg20") 
...
df %>%
  ...
  .$symbol -> for_label
df %>% filter(Sig != "N.S.")
for_label <- c( "Vrk3", "Trim12c", "Il18r1",  "Il5ra",  "Ptgs2", "Nfkb1", "Kpna4", "Rapgef1", "Irf1", "Trim5") 
...
ggsave("manuscript/brain/imgs/CytokineSignaling_25hr_use_use2.png",endo_plot,height =5, width =5, dpi=1000)
