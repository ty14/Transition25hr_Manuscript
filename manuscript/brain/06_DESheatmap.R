...
des70_upx <-  des %>% filter(time == 70) %>% 
  arrange(-dd_logFC) %>% head(.,10) %>% 
  select(symbol,time) 

des70_up$symbol %in% des70_upx$symbol

des70_down <-  des %>% filter(time == 70) %>% 
  arrange(dsub_logFC) %>% head(.,11) %>% 
  select(symbol,time )

des70_downx <-  des %>% filter(time == 70) %>% 
  arrange(-dd_logFC) %>% head(.,11) %>% 
  select(symbol,time )
des70_down$symbol %in% des70_downx$symbol
...
des25_upx <-  des %>% filter(time == 25) %>% 
  arrange(-dd_logFC) %>% head(.,10) %>% 
  select(symbol,time)

des25_up$symbol %in% des25_upx$symbol

des25_down <-  des %>% filter(time == 25) %>% 
  arrange(dsub_logFC) %>% head(.,10) %>% 
  select(symbol,time)

des25_downx <-  des %>% filter(time == 25) %>% 
  arrange(dd_logFC) %>% head(.,10) %>% 
  select(symbol,time)

des25_down$symbol %in% des25_downx$symbol
...
dd %>% filter(symbol %in% des25$symbol)
...
dd25 %>% filter(symbol %in% des70_down$symbol)
as25 %>% filter(symbol %in% des70_up$symbol)
...
ad70.log <- dd %>% filter(symbol %in% des.genes$symbol) %>% 
  select(symbol,`DES vs. DOM 70min` = logFC)
as70.log <- as %>% filter(symbol %in% des.genes$symbol) %>% 
  select(symbol,`DES vs. SUB 70min` = logFC)

des70 <- ad70.log %>% full_join(as70.log)
...
des25 <- ad25.log %>% full_join(as25.log)

des.hm <- des70 %>% full_join(des25)
