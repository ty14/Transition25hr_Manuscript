...
#filter
a_count <- a_count[rowSums(a_count >= 20), ]
...
pfc_data<- coldata %>%  na.omit() %>% filter(region != "A")
...
d = apply(a_count, 2, as.numeric)
dim(d)

#DGEList from limma
d0<-DGEList(d, group = amy_data1$condition1)
dim(d0)
rownames(d0) <- rownames(a_count) #redo rownames as get lost when making matrix
...
colData <- amy_data1 %>% rownames_to_column(var = "SampleName") %>% cbind(colData) 
...
ggsave("manuscript/brain/imgs/pca_mAMY_ALL.png", a, width = 12, height = 6)
