...
df %>%
  ...
  .$symbol -> for_label
for_label <- c("Hkdc1",  "Uqcc2",   "Ak4",  "Cox5a",  "Tmsb4x", 'Ndufaf1', "Prkag1")

df$logFC <-ifelse(df$symbol == "Hkdc1", -1.5, df$logFC)
EnhancedVolcano(df,
                lab = df$symbol,
                selectLab= for_label,
                x = 'logFC',
...
df %>%
  ...
  # filter(P.Value <0.01 | abs(logFC) >0.5) %>%
  .$symbol -> for_label
for_label <- c("Hkdc1", "Ak4", "Chchd10", "Cox5a",  "Bid", "Tsmb4x",  "Uqcc2",  "Atp5g3", "Cox6a1", "Nsufb8","Pgam2", "Zbtb20")  
df$logFC <-ifelse(df$symbol == "Hkdc1", -1.5, df$logFC)
EnhancedVolcano(df,
                lab = df$symbol,
                selectLab= for_label,
                ...
                cutoffLineType = 'blank',
                drawConnectors = TRUE, # just for a few plots
                widthConnectors = 0.01, # just for a few plots
                vline = c(-0.2, 0.2),
                vlineCol = c('grey90'),
                vlineType = c( 'dashed'),
                vlineWidth = c(0.3),
...
df %>%
  ...
  # filter(P.Value <0.01 | abs(logFC) >0.5) %>%
  .$symbol -> for_label
for_label <- c("Eno1","Dhtkd1", "Uqcc2",  "Nsufaf1",   "Tefm",   "Atp5a3",   "Sirt6", "Snca" )
...
df %>%
  ...
  # filter(P.Value <0.01 | abs(logFC) >0.5) %>%
  .$symbol -> for_label
for_label <- c("Dhtkd1","Uqcc2","Chchd10", "Cox6a1", "Atp5g3", "Tefm", "Ndufb8", "Slc2a6", "Uqcrc1")
...
#1 ATP Energy
top<- grid::textGrob("ATP Metabolism", gp = grid::gpar(fontsize = 10))
endo_plot <- gridExtra::grid.arrange(a,c,b,d, ncol =2)
ggsave("manuscript/brain/imgs/ATP_metabolic_70min_use3_forcedin15LOGFC.png",endo_plot,height =5, width =5, dpi=1000)
