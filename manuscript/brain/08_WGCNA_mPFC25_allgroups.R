...
dge.dl_dom <- dge.dl[, dge.dl$samples$group %in% c("ASC 25hr", "DES 25hr", "DOM 25hr", "SUB 25hr")]
...
pfc_data %>% 
  filter( condition1 %in% c("ASC 25hr", "DES 25hr", "DOM 25hr", "SUB 25hr") ) -> var_info  
...
var_info$condition1 %>% 
  factor(.,levels = c("DOM 25hr","ASC 25hr","DES 25hr", "SUB 25hr")) -> group.dl

group.dl<- gsub(" 25hr", "", group.dl)
...
pfc <- df %>% filter(time == 25) %>% filter(SampleID != "b1.2.1") %>% 
...
plotDendroAndColors(sampleTree2, traitColors,
                    ...
                    main = "mPFC:25hr dendrogram and trait heatmap")

collectGarbage()
#not going to filter because ascenders rreally are not grouping anyways?
saveRDS(datExpr,"manuscript/brain/results_use/WGCNA/WGCNA_datExpr25NORM_RG.RDS")
...
WGCNA_get_net <- function(my_tissue = "mPFC25hr",
  ...
  x <- readRDS("manuscript/brain/results_use/WGCNA/WGCNA_datExpr25NORM_RG.RDS") 
  set.seed(312)
  ...
  saveRDS(net, "manuscript/brain/results_use/WGCNA/WGCNA_net_mPFC25Power4.RDS")
...
net <- readRDS(glue::glue("manuscript/brain/results_use/WGCNA/WGCNA_net_mPFC25Power4.RDS"))
...
png(file = "manuscript/brain/imgs/cluster_dendo_mPFC25_Power4_minmod75.png",
...
saveRDS(MEs,"manuscript/brain/results_use/WGCNA/WGCNA_MEs_mPFC25Power4.RDS")
...
png(file = "manuscript/brain/imgs/module_trait_mPFC25Power4use.png",
...
saveRDS(wgcna_all, "manuscript/brain/results_use/WGCNA/WGCNA_WGCNA_MM_GS_mPFC25Power4.RDS")


##boxplots
pfc$SampleID <- paste0(pfc$SampleID, ".")
...
ggsave("manuscript/brain/imgs/boxplots_WGCNAPFC25NORM_RG.png", p1, width= 15, height = 5, dpi = 150)
...
p2
ggsave("manuscript/brain/imgs/boxplots_WGCNAPFC25NORM_RG_plot2.png", p1, width= 15, height = 5, dpi = 150)
...
write.csv(hubgenes_df, "manuscript/brain/results_tables/WGCNA/WCGNA_hubgene_list_mPFC25Power4.csv", row.names = F)
...
saveRDS(kIN_df,"manuscript/brain/results_use/kIN/kIN_dataframe_mPFC25_power4.RDS")
...
write.csv(wgcna_all_gos, 
          "manuscript/brain/results_tables/WGCNA/WGCNA_all_gos_catogeryBP_mPFC25_Power4.csv",
          row.names = F)
...
saveRDS(lm_result_all,"manuscript/brain/results_use/WGCNA/WGCNA_lm_result_mPFC25.RDS")
