...
EnhancedVolcano(df,
                ...
                cutoffLineType = 'blank',
                drawConnectors = TRUE, # just for a few plots
                widthConnectors = 0.09, # just for a few plots
                vline = c(-0.15, 0.15),
                vlineCol = c('grey90'),
                vlineType = c( 'dashed'),
                vlineWidth = c(0.3),
...
# #1 ATP Energy 
# top<- grid::textGrob("ATP Metabolism at 25 hr", gp = grid::gpar(fontsize = 10))
# endo_plot <- gridExtra::grid.arrange(a,b,c,d, ncol =2, top = top)
# ggsave("manuscript/brain/imgs/ATP_metabolic_25hr_use.png",endo_plot,height =5.5, width =5.5, dpi=1000)
