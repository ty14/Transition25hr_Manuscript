...
# getting trait data
colnames(coldata)
amy <- coldata[c(2,5,11,13,17,21,22,23)]
...
plotDendroAndColors(sampleTree2, traitColors,
                    ...
                    main = "PFC25:TRN dendrogram and trait heatmap(all samples)")

collectGarbage()
#not going to filter because ascenders rreally are not grouping anyways?
 # saveRDS(datExpr,"manuscript/brain/results/WGCNA_datExpr25_TRN.RDS")
...
WGCNA_get_net <- function(my_tissue = "mPFC25hr",
  ...
  x <- readRDS("manuscript/brain/results/WGCNA_datExpr25_TRN.RDS") 
  set.seed(312)
  ...
  saveRDS(net, "manuscript/brain/results/WGCNA_net_mPFC25_Power4_TRN.RDS")
...
net <- readRDS(glue::glue("manuscript/brain/results/WGCNA_net_mPFC25_Power4_TRN.RDS"))
...
png(file = "manuscript/brain/imgs/cluster_dendo_mPFC_Power4_TRN.png",
    width=600, height=350)
plotDendroAndColors(net$dendrograms[[2]], mergedColors[net$blockGenes[[2]]],
                    "Module colors",
                    dendroLabels = FALSE, hang = 0.03,
                    addGuide = TRUE, guideHang = 0.05, 
                    main = "mPFC 25hr TRN Power4")
...
set.seed(312)
datExpr <- readRDS("manuscript/brain/results/WGCNA_datExpr25_TRN.RDS") 
net <- readRDS("manuscript/brain/results/WGCNA_net_mPFC25_Power4_TRN.RDS")
dim(datExpr)
...
saveRDS(MEs,"manuscript/brain/results/WGCNA_MEs_mPFC25_Power4_TRN.RDS")
...
png(file = "manuscript/brain/imgs/module_trait_mPFC25_Power4_TRN.png",
...
saveRDS(wgcna_all, "manuscript/brain/results/WGCNA_WGCNA_MM_GS_mPFC25_Power4_TRN.RDS")
...
module = 'lightyellow'
module = 'darkgrey'
# Yellow, red,  purple , midnightblue, red, darkred, lightyellow, darkgrey,cyan, blue, black

module_list = c("black", "blue", "cyan", "lightyellow", "darkred", "midnightblue", "darkgrey", "purple", "red", "yellow")
...
write.csv(hubgenes_df, "manuscript/brain/results_tables/WCGNA_hubgene_list_mPFC25_Power4_TRN.csv", row.names = F)
...
saveRDS(kIN_df,"manuscript/brain/results/kIN/kIN_dataframe_mPFC25_Power4_TRN.RDS")
...
write.csv(wgcna_all_gos, 
          "manuscript/brain/results_tables/WGCNA_all_gos_catogeryBP_mPFC25_TRN.csv",
          row.names = F)

coldata <- coldata %>% rownames_to_column(., "SampleNames")

ME_df <-MEs%>% data.frame() %>% 
  tibble::rownames_to_column(var = "SampleNames") %>%
  pivot_longer(cols = 2:30, names_to = "Module") %>% 
