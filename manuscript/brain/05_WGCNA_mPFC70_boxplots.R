...
#agg recieved 
cd <- coldata %>% filter(condition1 != "ASC") %>% filter(condition1 != "DOM")
...
# Create a list to store linear model results
lm_result_list <- list()
...
#agg give
cs <- coldata %>% filter(condition1 != "DES") %>% filter(condition1 != "SUB")
...
me.l <- ME_df %>% pivot_longer(col=9:33) %>% split(.$name)
...
ME_df$MEdarkturquoise
lm_result_list %>% 
  do.call(rbind,.)%>% 
  filter(.,module != "grey")  %>% format(., scientific = F) -> d
d %>% filter(`Pr(>|t|)` < 0.05)
d %>% filter(module == "greenyellow")

lmer(MEgreenyellow ~ condition1*CORT +(1|batch)+(1|pre_id)+(1|post_id) , data = ME_df) -> mod1
lmer(module ~ condition2*CORT +(1|batch)+(1|pre_id)+(1|post_id) , data = df) -> mod2
lmer(module ~ condition3*CORT +(1|batch)+(1|pre_id)+(1|post_id) , data = df) -> mod3
summary(mod1)
summary(mod2)
