...
coldata <- coldata %>% 
  filter(region != "AMY") %>% 
  filter(Postrank != 3) %>% 
  filter(condition != "control") %>%  filter(condition1 != "ascenders")
...
saveRDS(v.dl, "manuscript/brain/results/limma_vdl_PFC70_TRN.RDS")
...
saveRDS(q.dl,("manuscript/brain/results/limma_vdl_cutoff5_2000_tworand_PFC70min_TRN.RDS"))

q.dl <- readRDS("manuscript/brain/results/limma_vdl_cutoff5_2000_tworand_PFC70min_TRN.RDS")
...
saveRDS(limma_list,"manuscript/brain/results/limma_PFC70_TRN.RDS")
