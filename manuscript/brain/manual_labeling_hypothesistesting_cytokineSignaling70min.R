...
df %>%
  ...
  .$symbol -> for_label
for_label <- c("Ppp2r1b", "Mapkapk2", "Vim", "Ifnar2", "Seh1l","Ptgs2","Gab2", "Aaas", "Ltbr", "Akt2", "Camk2d",
               "Ptk2b", "Rps6ka2", "Flnb", "Ccl12", "Il1b")

df %>% filter(Sig != "N.S.")
df$logFC <-ifelse(df$symbol == "Ccl12", 1.5, df$logFC)
df$logFC <-ifelse(df$symbol == "Il1b", 1.5, df$logFC)
EnhancedVolcano(df,
                lab = df$symbol,
                selectLab= for_label,
                x = 'logFC',
...
df %>%
 ...
 df %>% filter(Sig != "N.S.")
for_label <- for_label <- c( "Birc5", "Vim" ,"Ptgs2","Gab2", "Ltbr","Ptk2b", "Rps6ka2", "Flnb","Il1r1",  
                           "Cebpd","Ccl12",  'Stx1a',   'Il6st' ) 
...
df %>% filter(Sig != "N.S.")
for_label <- for_label <- c( "Birc5", "Ppp2r1b" ,"Seh1l",  "Aaas", "Camk2d", "Akt2", "Ifnar2", "Vbe2m", "Rnf7", "Arf1", "Psmb7" ) 
...
df %>%
  ...
  .$symbol -> for_label
df %>% filter(Sig != "N.S.")
for_label <-c( "Birc5",  "Aaas", "Vbe2m", "Rnf7","Arf1", "Psmb7", 'Il31ra', 'Flnb', 'Rps6ka2'  ) 
...
ggsave("manuscript/brain/imgs/CytokineSignaling_70min_use3_forcedLOGFC.png",endo_plot,height =5, width =5, dpi=1000)
