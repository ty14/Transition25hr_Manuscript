...
#### Pre-Organization Matrices, David's Scores, Transitivity etc.
...
p1 <- ggplot() + 
  geom_line(data=ds.reorg.long, aes(x=Rank, y=value, group=id), alpha=.3, color="gray57") +
  theme_classic() +
  xlab("") +
  ylab("David's Scores") +
...
p2 <- ggplot() + 
  ...
  geom_point(data=ds.reorg.summary25, 
             aes(x=Rank, y=median), size=3, shape=21, fill="white") +
  ggtitle("Prior Reorganization 25 hr") +
  ylim(-6,6)+
  theme(axis.text.x = element_text(vjust = 1,size = 20),
        # axis.ticks = element_blank(),
        axis.text.y = element_text(hjust = 0.5,size = 20),
...
ggsave("manuscript/behavior/results_figures/pre_ds.png",ds,height =6, width =15, dpi=600)
...
ggsave("manuscript/behavior/results_figures/pre_matrics.png",pm3,height =16, width =14, dpi=600)
