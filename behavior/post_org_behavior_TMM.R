...
df.groups <- df %>% select(post_batchcage, group, time) %>% unique

df.groups


# split l.mat into 'control' and 'reorganized'
ids <- df.groups$group[match(names(l.mat), df.groups$post_batchcage)]
...
p2 <- ggplot() + 
  ...
  geom_point(data=ds.control.summary, 
             aes(x=Rank, y=median), size=3, shape=21, fill="white") +
  ggtitle("Control Cages") +
  ylim(-6,6)


library(gridExtra)
ds <- grid.arrange(p1,p2, nrow=1)

ggsave("manuscript/behavior/results_figures/post_ds.png",ds,height =10, width =14, dpi=600)
